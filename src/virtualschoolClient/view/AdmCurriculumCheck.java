...
public class AdmCurriculumCheck {
...
//		User user =new User("09017421",User.UserStatus.ADMINISTRATOR,4);
//		ClientSocket cs=null;
//		EventQueue.invokeLater(new Runnable() {
//			public void run() {
//				try {
//					AdmCurriculumCheck window = new AdmCurriculumCheck(user,cs);
...
//	}
	 ...
	 */
	public AdmCurriculumCheck(User user,ClientSocket cs,JFrame frame) {
		this.frameCurChe=frame;
		this.cs=cs;
		this.user=user;
		initialize();
	}
	...
	private void initialize() {
		...
		back.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				AdmFunctionSelect frame=new AdmFunctionSelect(user,cs,frameCurChe);
			}
		});
		...
		comboBox.addItem("学年学期");
		comboBox.addItem("19-20-1");
	    comboBox.addItem("19-20-2");
	    comboBox.addItem("19-20-3");
	    
	    textField = new JTextField("输入学号");
		textField.setBounds(153, 42, 76, 21);
		panelCurChe.add(textField);
	    ...
	    comboBox.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent e) {
				...
				if (e.getStateChange() == ItemEvent.SELECTED) {
					String UserID = (String) textField.getText();
					String schSem = (String) comboBox.getSelectedItem(); 
					if(schSem=="学年学期") {
						return;
					}
					else if(UserID.equals("输入学号")) {
						JOptionPane.showMessageDialog(null, "请输入查询对象学号", "错误", JOptionPane.ERROR_MESSAGE);
						return;
					}
					List<Curriculum> curList = new CurriculumSendMessage().findCurriculumBySchSemAnduID(schSem,UserID,cs);
					
					if (curList.size() == 0 ) {
                        JOptionPane.showMessageDialog(null, "学号输入错误或此学年无课程", "错误", JOptionPane.ERROR_MESSAGE);
						return;
					}
	    ...
	    scrollPane.setViewportView(table);
	    scrollPane_1.setViewportView(table_1);
		comboBox.setBounds(40, 41, 90, 23); 
		panelCurChe.add(comboBox);
